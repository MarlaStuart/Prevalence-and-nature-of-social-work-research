...
#####################Didn't Use any of the rest
plot.STM(Fit4, type = "hist", xlim = c(0, .3))

out$meta$PredictedStudy <- as.factor(out$meta$PredictedStudy)
prep <- estimateEffect(1:6 ~ PredictedStudy, Fit6, meta = out$meta, uncertainty = "Global")

plot.estimateEffect(prep, covariate = "PredictedStudy", topics = c(1:6),
                       model = Fit6, method = "difference",
                       cov.value1 = "Not a Study", cov.value2 = "Study",
                       xlab = "Not a Study ... Study",
                       main = "Effect of study on research topic",
                       xlim = c(-.3, .3), labeltype = "custom",
                       custom.labels = c('Topic 1', 'Topic 2','Topic 3', 'Topic 4', 'Topic 5', 'Topic 6'))

perm <- permutationTest(formula =~ PredictedStudy, stmobj = Fit, treatment = "PredictedStudy", nruns = 5, documents = out$documents,
                        vocab = out$vocab, data = out$meta, stmverbose = FALSE, uncertainty = "Global")

#####Test models
prep <- estimateEffect(1:5 ~ PredictedStudy, Fit5, meta, nsims = 5)

#### Use covariates
out$meta$PredictedStudy <- as.factor(out$meta$PredictedStudy) ## not working
